...
class SendEmailBlast extends MarketingEvent {
  ...
  SendEmailBlast({required this.targetAudience, required this.subject, required this.bodyHtml});
  @override List<Object> get props => [targetAudience, subject, bodyHtml];
}
...
class SendPushBlast extends MarketingEvent {
  ...
  SendPushBlast({required this.targetAudience, required this.title, required this.message});
  @override List<Object> get props => [targetAudience, title, message];
}
...
class MarketingLoaded extends MarketingState {
  ...
  @override List<Object> get props => [campaigns, emailLogs, notificationLogs];
}
...
class MarketingBloc extends Bloc<MarketingEvent, MarketingState> {
  ...
  MarketingBloc({required this.dioClient}) : super(MarketingLoading()) {
    on<FetchMarketingData>((event, emit) async {
      ...
      try {
        final responses = await Future.wait([
          dioClient.dio.get('/api/marketing/campaigns'),
          dioClient.dio.get('/api/marketing/emails/history'),
          dioClient.dio.get('/api/marketing/notifications/history'),
        ]);

        final campaigns = (responses[0].data as List).map((j) => MarketingCampaignModel.fromJson(j)).toList();
        final emailLogs = (responses[1].data as List).map((j) => EmailLogModel.fromJson(j)).toList();
        final notificationLogs = (responses[2].data as List).map((j) => NotificationLogModel.fromJson(j)).toList();
    ...
    on<CreateCampaign>((event, emit) async {
      try {
        final res = await dioClient.dio.post('/api/marketing/campaigns', data: event.payload);
        emit(MarketingActionSuccess(res.data['message'] ?? 'Campaign created successfully.'));
    ...
    on<SendEmailBlast>((event, emit) async {
      try {
        final res = await dioClient.dio.post('/api/marketing/emails/send', data: {
          'targetAudience': event.targetAudience,
          'subject': event.subject,
          'bodyHtml': event.bodyHtml,
        });
        emit(MarketingActionSuccess(res.data['message'] ?? 'Email blast executed successfully.'));
        add(FetchMarketingData());
      } catch (e) {
        emit(MarketingError('Email dispatch failed. Verify MailerSend API settings.'));
      }
    });

    on<SendPushBlast>((event, emit) async {
      try {
        final res = await dioClient.dio.post('/api/marketing/notifications/send', data: {
          'targetAudience': event.targetAudience,
          'title': event.title,
          'message': event.message,
        });
        emit(MarketingActionSuccess(res.data['message'] ?? 'Push notification broadcasted.'));
        add(FetchMarketingData());
      } catch (e) {
